...
export default function EditAvaliacao() {
  ...
  const { data: avaliacao, isLoading: isLoadingAvaliacao, error: errorAvaliacao } = useQuery<Avaliacao>({
    queryKey: ["avaliacao", id],
    queryFn: () => id ? api.getAvaliacao(id) : Promise.reject("ID da avaliação não fornecido"),
    enabled: !!id,
  });
  ...
  useEffect(() => {
    if (avaliacao) {
      form.reset({
        leitoId: avaliacao.leito?.id || "",
        unidadeId: avaliacao.unidade?.id || "",
        scp: avaliacao.scp || "",
        colaboradorId: avaliacao.autor?.id || "",
  ...
  const updateAvaliacaoMutation = useMutation({
    mutationFn: (data: Partial<Avaliacao>) => id ? api.updateAvaliacaoSessao(id, data) : Promise.reject("ID da avaliação não fornecido"),
    onSuccess: () => {
    ...
    onError: (error) => {
      toast({
        title: "Erro",
        description: `Erro ao atualizar avaliação: ${error.message}`,
        variant: "destructive",
      });
    },
  });
  ...
  const onSubmit = (values: FormValues) => {
    updateAvaliacaoMutation.mutate(values);
  };

  if (isLoadingAvaliacao || isLoadingLeitos || isLoadingUnidades || isLoadingColaboradores || isLoadingScpMetodos) {
    return <div>Carregando Avaliação...</div>;
  }

  if (errorAvaliacao) {
    return <div>Erro ao carregar avaliação: {errorAvaliacao.message}</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Editar Avaliação</h1>
          <p className="text-muted-foreground">
            Edite os detalhes da avaliação de paciente.
          </p>
        </div>
      </div>
        ...
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <Button type="submit" disabled={updateAvaliacaoMutation.isPending}>
                {updateAvaliacaoMutation.isPending ? "Atualizando..." : "Atualizar Avaliação"}
              </Button>
            </form>
          </Form>
        </CardContent>
